...
- UI/动效再完善：
  - 流式回复期间状态栏文字轮播「思考 → 生成 → 回复」（600ms 定时器）。
  - 聊天区「正在思考…」打字指示器：流式开始后出现动态圆点气泡，首个内容分块到达时自动替换为正式气泡。
  - 新消息改为 250ms `OutCubic` 平滑滚动到底部，不再生硬跳变。
  - 消息气泡悬停高亮（`WA_Hover` + QSS `:hover`），助手/用户气泡边框与背景微调。
  - Windows 10/11 深色标题栏（DWM `DWMWA_USE_IMMERSIVE_DARK_MODE`，兼容 19/20 属性值），与深色主题融为一体。
  - 启动时聊天区展示欢迎语（纯客户端展示，不写入会话历史）。
  - `style.qss` 补充横向滚动条样式与状态栏渐变底色。
  - 编辑器欢迎页：无打开文件时显示「YCode」欢迎页，打开/新建文件自动移除，全部标签关闭后自动恢复；随面板主题联动。
  - 消息气泡新增「复制」按钮（悬停高亮，复制后短暂显示「已复制」）。
  - 发送按钮在输入为空时禁用，输入内容后自动启用。
  - 修复：流式输出期间每个分块到达都会把状态栏重置为「就绪」，与轮播文案冲突——现仅非流式输出时更新状态栏。
  - 修复输入框占位提示与实际行为不一致（Enter 发送）。
