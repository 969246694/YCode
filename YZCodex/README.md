...
新建游戏项目会生成 CMake 入口、`src/main.cpp`、`scenes/main.scene.json`、`assets/` 和 `plugins/`。示例代码会让 YCodeEngine 从 `scenes/main.scene.json` 加载 `Player` 实体，在窗口中绘制它，并通过方向键或 WASD 更新它的 `Transform2D`。
